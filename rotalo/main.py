...
# data = dr.generate_mock(12.,0.001,4,0.001,0.5,0.001,-50.,0.001,150.,0.001,0.,0.0001,-2.,-1.,20)
# data.MOCK_FeH()
...
# for i in range(0,20):
#     v_pos = np.array([data.X[i]-C.X_sun,data.Y[i],data.Z[i]])
...
print('mean R is ',np.mean(R_o))
print('mean RV is ',np.mean(rv_o))
print(len(l_o[l_o>180]))
...
for iR in range(0,1):#Nr):
    print(iR)
    min_R = 0#Rarray[iR]
    max_R = 25#Rarray[iR+1]
    for iZ in range(0,1):#Nz):
        min_Z = -5#Zarray[iZ]
        max_Z = 20#Zarray[iZ+1]
        ind = (Z_o>min_Z) & (Z_o<max_Z) & (R_o>min_R) & (R_o<max_R) & (l_o>250) & (l_o<290)# & (np.abs(Z_o)<4)
        nmb[iZ, iR] = len(Z_o[ind])
        print('nmb is ',nmb[iZ,iR])
        if (nmb[iZ,iR]>10):# & (i_check<1):
            ...
            for i in range(0,Nv+1):
                ...
                for j in range(0,Nd+1):
                    ...
                    Vz_star = sVphi*0.-1 * C.W_sun
                    Px_star = R * np.cos(Lgc * math.pi / 180) - C.X_sun
                    Py_star = R * np.sin(Lgc * math.pi / 180)
                    Pz_star = Z
                    # print(Px_star[0],Py_star[0],Pz_star[0])
                    # print(len(Vy_star))
                    # print(sVphi,Vx_star[0],Vy_star[0],Vz_star)
